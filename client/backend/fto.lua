...
RegisterNUICallback('getFTOList', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:getFTOList', page, filters)
...
RegisterNUICallback('getFTO', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:getFTO', data.id)
...
RegisterNUICallback('getOfficerFTOHistory', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:getOfficerFTOHistory', data.officerCitizenId)
    cb(result or {})
end)
...
RegisterNUICallback('createFTOAssignment', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:createFTOAssignment', data)
    cb(result or { success = false })
end)
...
RegisterNUICallback('updateFTOAssignment', function(data, cb)
    if not MDTOpen then cb({ success = false }) return end
    if not data or not data.id then cb({ success = false }) return end
    local result = MDT.callback(resourceName .. ':server:updateFTOAssignment', data.id, data.updates or {})
    cb(result or { success = false })
end)
...
RegisterNUICallback('deleteFTOAssignment', function(data, cb)
    if not MDTOpen then cb({ success = false }) return end
    if not data or not data.id then cb({ success = false }) return end
    local result = MDT.callback(resourceName .. ':server:deleteFTOAssignment', data.id)
    cb(result or { success = false })
end)
...
RegisterNUICallback('createFTODor', function(data, cb)
    if not MDTOpen then cb({ success = false }) return end
    if not data then cb({ success = false }) return end
    local result = MDT.callback(resourceName .. ':server:createFTODor', data)
    cb(result or { success = false })
end)
...
RegisterNUICallback('deleteFTODor', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:deleteFTODor', dorId)
    cb(result or { success = false })
end)

RegisterNUICallback('getFTOPhases', function(data, cb)
    if not MDTOpen then cb({}) return end
    local result = MDT.callback(resourceName .. ':server:getFTOPhases')
    cb(result or {})
end)
...
RegisterNUICallback('saveFTOPhases', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:saveFTOPhases', data.phases)
    cb(result or { success = false })
end)

RegisterNUICallback('getFTOCompetencies', function(data, cb)
    if not MDTOpen then cb({}) return end
    local result = MDT.callback(resourceName .. ':server:getFTOCompetencies')
    cb(result or {})
end)
...
RegisterNUICallback('saveFTOCompetencies', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:saveFTOCompetencies', data.competencies)
    cb(result or { success = false })
end)
...
RegisterNUICallback('advanceFTOPhase', function(data, cb)
    ...
    cb(MDT.callback(resourceName .. ':server:advanceFTOPhase', data or {}) or { success = false })
end)

RegisterNUICallback('setFTOStatus', function(data, cb)
    if not MDTOpen then cb({ success = false, error = 'MDT is not open' }) return end
    cb(MDT.callback(resourceName .. ':server:setFTOStatus', data or {}) or { success = false })
end)