...
RegisterNUICallback('setCallsign', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:setCallsign', {
...
RegisterNUICallback('getCallsign', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:getCallsign', {
...
RegisterNUICallback('setRadio', function(data, cb)
    ...
    local result = MDT.callback(resourceName .. ':server:setRadio', {
        citizenid = data.cid or data.citizenid,
...
RegisterNetEvent(resourceName .. ':client:setRadio', function(radio)
    if type(tonumber(radio)) == 'number' then
        ...
        if success then
            MDT.notify('Radio frequency set to ' .. radio, 'success')
        else
            MDT.notify('Failed to set radio - pma-voice not available', 'error')
        end
    else
        MDT.notify('Invalid radio frequency', 'error')
    end
end)
...
RegisterNUICallback('setWaypointU', function(data, cb)
    ...
    local coords = MDT.callback(resourceName .. ':server:getUnitLocation', data.cid)
    if coords then
        SetNewWaypoint(coords.x, coords.y)
        MDT.notify('GPS set to officer location', 'success')
    else
        MDT.notify('Officer not found or offline', 'error')
    end
    cb('ok')
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    if data.coord and data.coord[1] then
        ...
        if coords[1] and coords[2] then
            ...
            MDT.notify('GPS set to property location', 'success')
        end
